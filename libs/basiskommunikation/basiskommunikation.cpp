...
void  Basiskommunikation::sendmessage(const char* topic, const byte* msgbuffer, uint8_t len)  //Binärzeugs verschicken
{
	...
	strncpy(newtopic, _MQTTNAME, lenN);
	newtopic[lenN] = '/';
	strncpy(newtopic + lenN + 1, topic, lenT + 1);
...
void Basiskommunikation::update() {
	...
	while (Serial.available()) {
		...
		if (ziffernzaehler >= sizeof(zahl) - 1) // mind. ein Platz für Nullterminierung
		{
			Serial.println(F("Puffer überschritten, beginne neu! Bisher gesendetes wird gelöscht!"));
			ziffernzaehler = 0;
