...
// Template: Pin kommt als Nicht-Typ-Template-Parameter
...
private:
    ...
    myUhrBasis* _Uhrzeit;
    myTimer     Uhrtimer;
    uint16_t    Uhrrefreshzeit    = 10000;
    myTimer     DyntextTimer;
    uint16_t    DynTextrefreshzeit = 50;

public:
    // Pin brauchen wir nicht mehr im ctor, der ist ja im Template
    myLEDMatrix(myUhrBasis* Uhrzeit);
...
template <uint8_t DATA_PIN>
void myLEDMatrix<DATA_PIN>::drawText(const char* text)
{
    // Matrix löschen
            ...
            for (uint8_t row = 0; row < 7; row++) {
                if (colBits & (1 << row)) {
                    ...
                    if ((xOffset + col) < _width && y < _height) {
                        _leds[xyToIndex(xOffset + col, y)] = CRGB(255, 0, 0);
                    }
                }
            }
        }
...
template <uint8_t DATA_PIN>
void myLEDMatrix<DATA_PIN>::drawTextDyn()
{
    // Matrix löschen
    for (int i = 0; i < _Pixelanzahl; i++)
        _leds[i] = CRGB::Black;
    ...
    if (_anzahllauftext == 0) {
        // keine neue Anzeige -> altes Bild bleibt im Buffer
        return;
    }
                ...
                for (uint8_t row = 0; row < 7; row++) {
                    if (colBits & (1 << row)) {
                        ...
                        if (x >= 0 && x < _width && y < _height) {
                            _leds[xyToIndex(x, y)] = CRGB(255, 0, 0);
                        }
                    }
                }
            }
...
template <uint8_t DATA_PIN>
void myLEDMatrix<DATA_PIN>::drawUhrzeit()
{
    // Achtung: getTime(...) liefert vermutlich ein temporäres Objekt.
    // Wenn das bei dir ein std::string ist, wäre es sauberer, das erst zu speichern.
    drawText(_Uhrzeit->getTime("%R").c_str());
}
template <uint8_t DATA_PIN>
void myLEDMatrix<DATA_PIN>::update()
{
    if (_mode == 'U') {
        // Uhr-Modus
...
template <uint8_t DATA_PIN>
myLEDMatrix<DATA_PIN>::myLEDMatrix(myUhrBasis* Uhrzeit)
: _Uhrzeit(Uhrzeit)
{
    memset(textBuffer, 0, sizeof(textBuffer));
    strcpy(textBuffer, "      ");

    debugln("myLEDMatrix Template-Constructor called");

    // Jetzt: Pin kommt aus dem Template
    FastLED.addLeds<WS2812B, DATA_PIN, GRB>(_leds, _Pixelanzahl);
...
template <uint8_t DATA_PIN>
uint16_t myLEDMatrix<DATA_PIN>::xyToIndex(uint8_t x, uint8_t y) {
    if (x % 2 == 0) {
        return x * _height + y;
    } else {
...
template <uint8_t DATA_PIN>
bool myLEDMatrix<DATA_PIN>::callbackismineanddo(char* topic, byte* payload, unsigned int length)
{
    ...
    FastLED.setBrightness((uint8_t)hellVal);
    ...
    // Text (ab Position 8 bis max 99 Zeichen)
