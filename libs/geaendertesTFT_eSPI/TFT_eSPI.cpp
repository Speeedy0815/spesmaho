...
// In parallel mode and with the RP2040 processor, the TFT_WR line is handled in the  PIO
	...
	if (_restpin >= 0) {
		...
		digitalWrite(_restpin, HIGH); // Set high, do not share pin with another SPI device
	}
...
***************************************************************************************/
void TFT_eSPI::begin(int8_t restpin, int8_t csPin, uint8_t tc)
{
	init(restpin, csPin, tc);
}
...
***************************************************************************************/
void TFT_eSPI::init(int8_t restpin, int8_t csPin, uint8_t tc)
{
	_restpin = restpin;
	_csPin = csPin;
	if (_booted)
	{
		...
		cspinmask = (uint32_t)digitalPinToBitMask(_csPin);
...
// It shows that the padding areas are being correctly sized and positioned
	...
	***************************************************************************************/
	void TFT_eSPI::getSetup(setup_t& tft_settings)
	{
		...
		tft_settings.pin_tft_cs = _csPin;
		...
		tft_settings.pin_tft_rst = _restpin;
