    ...
    uint8_t anzSpal, uint8_t anzZeil, uint8_t* Spaltlst, uint8_t* Zeillist, uint8_t* Keyliste)
{
    ...
    ResTaster = new myInput*[_anzSpal * _anzZeil];
    for (uint8_t i = 0; i < _anzSpal; i++)
    {
        for (uint8_t j = 0; j < _anzZeil; j++)
        {
            ResTaster[i * _anzZeil + j] = new myInput(_mqtt, Keyliste[j * _anzSpal + i], true);
            ResTaster[i * _anzZeil + j]->begin(false);
        }
    }
...
void myTastermatrix::update()
{
    ...
    for (uint8_t i = 0; i < _anzSpal; i++)
    {
        ...
        for (uint8_t j = 0; j < _anzZeil; j++)
        {
            ...
            //Serial.println(erg);
            uint8_t mtxnr = i * _anzZeil + j;  //J = Zeile
...
bool myTastermatrix::callbackismineanddo(char* topic, byte* payload, unsigned int length)
{
    for (uint8_t i = 0; i < _anzSpal; i++)
    {
        for (uint8_t j = 0; j < _anzZeil; j++)
        {
            if (ResTaster[i * _anzZeil + j]->callbackismineanddo(topic, payload, length))
            {
                return true;
            }
        }
